...
std::queue<float32_t*> out_data_buffer;
int max_buf_size = 65536;
// float32_t *output_data_buf[30000] = {NULL}; 
...
//    }
    ...
    float32_t *data_frame_normalized = (float32_t *) malloc(fdata_count * sizeof(float32_t));
    status = x4driver_read_frame_normalized(x4driver, &frame_counter, data_frame_normalized, fdata_count);
    
    out_data_buffer.push(data_frame_normalized);
/*
...
/*
    ...
    {
        for (int print_frame = 0; print_frame < frame_counter; print_frame++)
        {   
            if(output_data_buf[print_frame] != NULL)
            {
                printf("Size:%d,New Frame Data Normolized(%d){", fdata_count, print_frame);
                for (int i = 0; i < fdata_count; i++)
                {
                    printf(" %f, ", output_data_buf[print_frame][i]);
                }
                printf("}\n");
            }
        }
    ...
    {
        output_data_buf[frame_counter-1] = data_frame_normolized;
    }
*/

}

void clearQueue(std::queue<float32_t*> &q )
{
   std::queue<float32_t*> empty;
   std::swap(q, empty);
}

void printArray(float32_t *arr, int length)
{
...
{   
    ros::NodeHandle n;
    ros::Publisher sender_pub = n.advertise<timeArr_msgs::FloatArray>("uwb_chunk", 1000);
    
    while(ros::ok())
    {

        // std::cout << out_data_buffer.size()  << std::endl;
        // std::cout << out_data_buffer.size() << std::endl;
        if (out_data_buffer.size() >= max_buf_size)
        {
            // out_data_buffer.clear(); // For memory safety
            clearQueue(out_data_buffer);
            std::cout << "Buffer Cleaned\n";
            
        }
        int chunk_size = 1024;
        // std::cout << "Buffer Size: " << out_data_buffer.size() << std::endl;
        if (out_data_buffer.size() >= chunk_size) // send the chunk
        {
            
            int bin_count = 120; // need to be changed based on your setup
            int iq_count = bin_count * 2;
            // float32_t data_chunk[iq_count * chunk_size];
            std::vector<float>uwb_chunk(iq_count * chunk_size);
            //std::cout << "Chunk published" << std::endl;
            
            for (int curr_frame = 0; curr_frame < chunk_size; curr_frame++)
            {
                
                //std::cout << out_data_buffer.size() << std::endl;
                //memcpy( data_chunk + curr_frame * iq_count, out_data_buffer.front(), sizeof(float32_t) * iq_count );
                memcpy( &uwb_chunk[0] + curr_frame * iq_count, out_data_buffer.front(), iq_count * sizeof(float));
                //printArray(out_data_buffer.front(), iq_count);
                free(out_data_buffer.front());
                //std::cout<<out_data_buffer.front()<<std::endl;
                out_data_buffer.pop();
            ...
            for (int curr_frame = 0; curr_frame < chunk_size; curr_frame++)
            {
                //float[] temp_arr = out_data_buffer.front();
                float *temp_arr = out_data_buffer.front();
                for (int j = 0; j < iq_count; j++) {
                    float temp_val = *(temp_arr + j);
                   // cout << temp_val << endl;
                    uwb_chunk.push_back(temp_val);
                }
            ...
            */
            ...
            // printArray(data_chunk, iq_count * chunk_size);
            timeArr_msgs::FloatArray msg;
            long seconds = ros::Time::now().toSec();
            long ns = (int) ( (ros::Time::now().toSec() - (int) ros::Time::now().toSec()) * 1e9);
            msg.header.stamp.sec = seconds;
            msg.header.stamp.nsec = ns;
            msg.data = uwb_chunk;
            sender_pub.publish(msg);
            std::cout << "Chunk Sent!\n";
            /*
            for (int iii=0; iii<uwb_chunk.size(); iii++ )
            {
                cout << uwb_chunk[iii] << endl; 
            }
            */
        }
...
uint32_t task_radar_init(X4Driver_t **x4driver)
{
    ...
    uintptr_t type_casting_temp = reinterpret_cast<uintptr_t>(task_parameters->x4driver->spi_buffer);
    if ((((uint32_t)type_casting_temp) % 32) != 0)
    {
        int alignment_diff = 32 - (((uint32_t)type_casting_temp) % 32);
...
int taskRadar(void)
{
...
#endif
    status = x4driver_set_iterations(x4driver, xgl_iter);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
...
#endif
    status = x4driver_set_pulses_per_step(x4driver, xgl_pulsestep);
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
...
#endif
    status = x4driver_set_downconversion(x4driver, xgl_getiq); // Radar data as downconverted baseband IQ, not RF.
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
    ...
    status = x4driver_set_frame_area(x4driver, min_dist, max_dist); // Observe from 0.5m to 4.0m.
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
#ifdef DEBUG
    ...
    status = x4driver_set_tx_power(x4driver, xgl_tx_power);
#ifdef DEBUG
    if (status == XEP_ERROR_X4DRIVER_OK)
    {
    ...
    {
        printf("x4driver_set_tx_power unknow situcation\n");
    }
#endif // DEBUG
